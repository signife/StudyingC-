...
public:
    ...
    void push_back(bool s)
    {
        ...
        {
            unsigned int *temp = new unsigned int[capacity * 2]; // temp* 타입 만들고
            for (int i = 0; i < capacity; i++)
            {
                temp[i] = data[i]; // temp가 가르키는거에 data대입 하고
            }
            for (int i = capacity; i < 2 * capacity; i++)
            {
                temp[i] = 0;
            }
            delete[] data; // data가 가르키던 구식 메모리 해제
            data = temp;   // data가 다시 temp를 가르키게 함
            capacity *= 2;
        }
